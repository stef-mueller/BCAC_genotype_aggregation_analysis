---
title: "S04.18.FAM53B.plot_gene_region"
...
```{r}
genes_diverse=c("RP11-231C18.3","SRGAP2C","MAP3K1","FAM175A","BTN2A1",
                "MIER3","TNNT3","LSP1","FMNL3","FAM72B",
                "FGFR1","ZNF404","LYPD5","KCNN4", "FGFR2",
                "CBLB","ESR1", "FAM53B")

id_diverse=c(2312,2578,3587,5340,6639,
             8691,10113,11289,11889,13017,
             13061,13984,16771,16809,2055,
             2294,6631,9438)

genes_euro=c("FAM72B","SRGAP2C","RP11-231C18.3","FAM175A","MAP3K1",
             "MIER3","BTN2A1","FGFR1","LSP1","TNNT3",
             "FMNL3","KCNN4","LYPD5","ZNF404","FGFR2",
             "CBLB","ESR1", "FAM53B")

id_euro=c(1059,1061,14069,14226,14742,
          14744,15600,17592,2810,2811,
          4385,9802,9803,9805,2665,
          13319,16397,2692)
...
# Create individual gene data: FAM53B
## Load MONSTER SNPlist
### Set Gene
```{r}
CURRENT_GENE = gene_id_map %>% filter(gene == "FAM53B")
```
...
gene_model = query_genes_in_region_biomart_hg38(
  ...
  mart = ensembl)
# only keep gene_model elements with HGNC Symbol
...
pf3 = ggplot(bcac_current.lift,
  ...
  geom_hline(aes(yintercept = monster_current$neg_log_meta_euro_raw),
                 linetype="dashed",
                 color=mycols[2]) +
  geom_hline(aes(yintercept = -log10(0.0000786)), #unadjusted pvalue in african cohort
             color="forestgreen")
```
