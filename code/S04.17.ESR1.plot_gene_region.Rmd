---
title: "S04.17.ESR1.plot_gene_region"
...
```{r}
# test whether on Dropbox of local Ubuntu laptop
if (!grepl("Ubuntu",system("uname -v", intern=T))){
  DIR="/home/rstudio/WORK/UCL/UGI/BCAC_publication"
} else {
  DIR="/home/stef/Dropbox/WORK/UCL/UGI/projects/breast_cancer/publication"
}
...
```{r}
genes_diverse=c("RP11-231C18.3","SRGAP2C","MAP3K1","FAM175A","BTN2A1","MIER3","TNNT3","LSP1","FMNL3","FAM72B","FGFR1","ZNF404","LYPD5","KCNN4", "FGFR2","CBLB","ESR1")

id_diverse=c(2312,2578,3587,5340,6639,8691,10113,11289,11889,13017,13061,13984,16771,16809,2055,2294,6631)

genes_euro=c("FAM72B","SRGAP2C","RP11-231C18.3","FAM175A","MAP3K1","MIER3","BTN2A1","FGFR1","LSP1","TNNT3","FMNL3","KCNN4","LYPD5","ZNF404","FGFR2","CBLB","ESR1")
...
# Create individual gene data: ESR1
## Load MONSTER SNPlist
### Set Gene
```{r}
CURRENT_GENE = gene_id_map %>% filter(gene == "ESR1")
```
...
```{r}
print("hg19")
paste0(unique(all_cohorts$CHROM[!is.na(all_cohorts$CHROM)]),":",min(all_cohorts$pos_hg19, na.rm = T),"-",max(all_cohorts$pos_hg19, na.rm = T))

print("hg38")
paste0(unique(all_cohorts$CHROM[!is.na(all_cohorts$CHROM)]),":",min(all_cohorts$pos_hg38, na.rm = T),"-",max(all_cohorts$pos_hg38, na.rm = T))
```
...
gene_model = query_genes_in_region_biomart_hg38(
  gene = CURRENT_GENE$gene,
  region = paste0(unique(all_cohorts$CHROM[!is.na(all_cohorts$CHROM)]),":",min(all_cohorts$pos_hg38, na.rm = T),":",max(all_cohorts$pos_hg38, na.rm = T)),
  mart = ensembl)
```
...
bcac_current = bcac_gwas %>%
  filter(Chrom == all_cohorts$CHROM[!is.na(all_cohorts$CHROM)][1] &
...
# override xlim fir ESR1 to make plot easier to understand, as no variants outside of
# this manual window have been included in ESR1 gene aggregation model
xlim = c(151600000, 152170000)
...
```{r}
pf1 =  gene_model %>%
  # remove SYNE1 gene from plot as region is no longer included
  filter(external_gene_name == "ESR1") %>% 
  ggplot(aes(xmin = start_position, xmax = end_position,
                             y = external_gene_name, 
                             fill=gene_of_interest,
                             forward=direction)) +
  geom_gene_arrow() +
  scale_fill_manual(values = rev(mycols),guide=F)+
