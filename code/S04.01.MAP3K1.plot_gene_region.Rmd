...
# Create individual gene data: MAP3K1
## Load MONSTER SNPlist
### Set Gene
```{r}
CURRENT_GENE = gene_id_map %>% filter(gene == "MAP3K1")
```
...
```{r}
print("hg19")
paste0(unique(all_cohorts$CHROM),":",min(all_cohorts$pos_hg19),"-",max(all_cohorts$pos_hg19))

print("hg38")
paste0(unique(all_cohorts$CHROM),":",min(all_cohorts$pos_hg38),"-",max(all_cohorts$pos_hg38))
```
...
gene_model = query_genes_in_region_biomart_hg38(
  gene = CURRENT_GENE$gene,
  region = paste0(unique(all_cohorts$CHROM),":",min(all_cohorts$pos_hg38),":",max(all_cohorts$pos_hg38)),
  mart = ensembl)
```
...
bcac_current = bcac_gwas %>%
  filter(Chrom == all_cohorts$CHROM[1] &
         pos_hg19 >= min(all_cohorts$pos_hg19) & 
         pos_hg19 <= max(all_cohorts$pos_hg19)) %>% 
  dplyr::select(-Chrom) %>% 
  distinct()
...
## general variables
```{r}
xlim=c(min(all_cohorts$pos_hg38, gene_model$start_position),max(all_cohorts$pos_hg38, gene_model$end_position))
